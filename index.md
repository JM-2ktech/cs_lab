...
<!-- 저작 메모(2026-08-13 전면 재작성): 구 opening.md를 흡수한 홈 페이지다.
     ...
     ★ 이 페이지는 랩이 전부 확정된 뒤에 마지막으로 맞춘다.
       ...
       존재하지 않는 Lab 7을 가리키고 있었다(2026-08-13 발견). 인덱스를 먼저 쓰고
       랩을 고치면 같은 일이 되풀이된다.
...
{: .note }
[참고. 에이전트 안에서 일어나는 일](./docs/reference-inside-the-agent.html)은 랩이 아니라 읽고 짧게 실습하는 페이지입니다. **1부**([RAG](./docs/glossary.html#rag)·[인덱싱](./docs/glossary.html#indexing))는 Lab 2의 인덱싱 대기 시간에 흡수되어 추가 시간이 들지 않습니다. **2부**(토큰·비용)는 별도 15분 슬롯으로 편성합니다 — 앞뒤 랩과 의존이 없어 어디에 끼워도 되고, 시간이 밀리면 통째로 뺄 수 있습니다. **3부**(지식 문서 준비)는 10분이고 읽는 자리입니다.
