...
<!-- 저작 메모: mail-agent-selfbuild-tutor 스킬의 L0(사전점검)~L3(테스트 시나리오) 정의를
     ...
     2026-08-10 촬영 실측으로 보정한 라벨(스텝 3~6):
       ...
       - "대화 시작 예시(Starter prompts)" → **추천 프롬프트**, 제목/메시지 2칸 구조 (lab1-05b 추가)
...
5. 이름 바로 아래 **에이전트 설명하기** 자리를 클릭해 아래 한 줄을 붙여넣습니다.
    ...
    이어서 아래로 스크롤해 **추천 프롬프트** 섹션에 2개를 등록합니다. **제목**과 **메시지** 칸이 따로 있으니 각각 붙여넣습니다.
    ...
    **완료 기준**: 추천 프롬프트 목록에 2줄이 등록되고, 그 아래 빈 입력칸이 한 줄 남는다.
    ...
    ![추천 프롬프트 2개 등록 — 제목과 메시지 2칸](../assets/lab1/lab1-05b.png)
...
7. 아래로 스크롤해 **지식** 섹션으로 갑니다. `에이전트가 응답을 생성하는 데 사용할 원본 선택` 아래 **파일, 모임, 채팅, 이메일 및 웹 사이트 추가**에 아이콘이 몇 개 놓여 있습니다.
    ...
    {: .warning }
    **이 스텝을 건너뛰면 다음 스텝에서 메일 요약이 안 됩니다.** 지침에 "받은 편지함의 메일을 확인하고 요약합니다"라고 써두는 것만으로는 부족합니다 — 에이전트가 **무엇을 볼 수 있는지**는 지침이 아니라 [지식](./glossary.html#knowledge)에서 정해집니다.
...
12. 안내가 다 들어갔으면 상단 **구성** 버튼을 눌러 드롭다운을 열고 **사용해 보기**를 선택합니다. 편집 화면과 테스트 화면은 이 드롭다운 하나로 오갑니다 — 앞으로 지침을 고칠 때마다 이 두 화면을 왕복하게 됩니다.
    ...
    **완료 기준**: 테스트 화면으로 바뀌고, 5번에서 등록한 **추천 프롬프트 2개**가 카드로 보인다. 그 아래 입력창에 직접 문장을 칠 수도 있습니다.
...
## 부록. 내 에이전트를 Copilot Studio로 가져가기
...
방금 만든 **메일 보조**는 [Agent Builder](./glossary.html#agent-builder)로 만든 에이전트입니다. 이걸 **[Copilot Studio](./glossary.html#copilot-studio)로 복사**하면, [Lab 2](./lab2.html)부터 쓰게 될 도구에서 같은 에이전트가 어떻게 보이는지 비교할 수 있습니다. 복사이므로 원본은 그대로 남습니다.

1. 오른쪽 위 `...` 메뉴를 열고 **Copilot Studio에 복사**를 선택합니다.

    ![더보기 메뉴에서 Copilot Studio에 복사 선택](../assets/lab1/lab1-a01.png)

2. **Copilot Studio의 정식 버전으로 복사할까요?** 대화상자가 뜹니다. **무엇이 따라오고 무엇이 안 따라오는지**가 여기 적혀 있습니다.

    ![복사 확인 대화상자 — 포함·미포함 항목](../assets/lab1/lab1-a02.png)
    ...
    {: .warning }
    **7번에서 켠 「내 이메일」은 따라오지 않습니다.** 안내에 쓴 지침은 한 글자도 빠짐없이 넘어가는데 지식 원본만 빠집니다 — 복사본은 **메일을 못 읽습니다.** 8번에서 본 "권한이 있다고 알아서 보는 게 아니다"가 여기서 한 번 더 나옵니다. **지침은 옮겨져도, 무엇을 볼지는 새 자리에서 다시 정해 주어야 합니다.**

    **시작**을 누릅니다.

3. **환경 선택**이 뜹니다. 강사가 안내하는 환경을 고르고 **계속**을 누릅니다.

    ![환경 선택 화면](../assets/lab1/lab1-a03.png)
...
4. **에이전트 구축 시작** 화면으로 넘어갑니다. 이름이 **메일 보조(복사)** 로 붙고 설명·지침이 그대로 들어와 있습니다. 오른쪽 위 **만들기**를 누릅니다.
    ...
    ![에이전트 구축 시작 — 이름·설명·지침이 복사된 상태](../assets/lab1/lab1-a04.png)

5. 완성되면 에이전트 화면이 열립니다. 같은 에이전트인데 **없던 것들이 붙어 있습니다.** 지금은 눈으로만 확인합니다.

    ![Copilot Studio의 에이전트 화면 — 모델 선택·참조 자료·도구·게시](../assets/lab1/lab1-a05.png)
...
{: .important }
**같은 에이전트인데 다룰 수 있는 게 늘었습니다.** Agent Builder는 "지침으로 부탁하기"까지가 전부였고, 16번에서 그 한계를 직접 봤습니다. Copilot Studio에는 그 한계를 넘는 자리들이 있습니다 — 부탁 대신 순서를 정하는 **토픽**, 읽기만 하던 것을 넘어서는 **도구**. 오늘 남은 랩은 이 자리들을 하나씩 채워 가는 과정입니다.
