...
## 시작하기 전에 — 지식을 넣는 두 번째 길

{: .time }
5분 · 읽고 넘어갑니다.

[Lab 2](./lab2.html)에서는 **파일을 올렸습니다.** 우리가 가진 문서를 넣고, 그 안에서만 답하게 했습니다.

그런데 회사 밖에 있는 정보는요? 법·제도·공공 기준 같은 것들은 우리가 파일로 갖고 있지 않고, 갖고 있어도 금방 낡습니다.

Lab 2 마지막에 이런 답을 받았습니다.

```
퇴직금은 얼마나 나오나요?
→ 현재 사내 자료에서 퇴직금에 관한 구체적인 규정이나 계산 기준을 찾을 수 없었습니다
```

문서에 없으니 당연합니다. **그러면 문서를 늘리면 됩니다.** 이번에는 파일이 아니라 **웹사이트**로요.

{: .important }
이 랩에서는 같은 질문을 **세 사이트로** 물어봅니다. 셋 다 답은 나옵니다. 다른 것은 **무엇을 근거로 답하느냐**입니다. 그걸 화면에서 가려내는 것이 오늘의 목표입니다.
...
14. 다시 **편집**을 눌러 마지막 주소로 바꿉니다. 법제처가 운영하는 「찾기 쉬운 생활법령정보」입니다.
...
15. **새 테스트 세션**에서 **세 번째로 같은 질문**을 합니다.
    ...
    **길이로 비교하지 마세요.** 블로그 쪽이 더 길 수도 있습니다. 달라진 것은 **논제**입니다. 블로그는 「실제로 어떻게 받는가」를 사례와 팁으로 풀고, 법제처는 **「법이 뭐라고 정하고 있는가」**를 조문 번호와 함께 말합니다. 둘 다 쓸모가 있지만, **조문 번호는 확인할 수 있고 팁은 확인할 수 없습니다.**
    ...
    {: .note }
    **에이전트가 갑자기 똑똑해진 게 아니라 다른 문서를 만난 것**입니다. [참고 페이지 3부](./reference-inside-the-agent.html)에서 말한 「문서가 답을 정한다」가 그대로 보이는 자리입니다 — 같은 모델, 같은 질문, 같은 지침인데 근거가 바뀌니 답의 성격이 바뀝니다.
