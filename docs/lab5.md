...
{: .important }
**읽는 손과 쓰는 손은 무게가 다릅니다.** 읽기는 틀려도 화면 안에서 끝나지만, 쓰기는 외부의 결과가 바뀝니다. 이 랩은 읽기로 시작해 쓰기로 끝나고, **마지막에 그 차이가 무엇을 뜻하는지** 직접 보게 됩니다.
...
7. **세부 정보** 화면이 열립니다. **설명**을 아래로 바꿔 씁니다.
    ...
    **완료 기준**: 설명이 우리 용도로 바뀌었다. 아래 **도구**·**연결**·**대상**에 Excel 커넥터와 본인 계정, 그리고 이 에이전트가 표시된다.
    ...
    **사용할 자격 증명**은 날씨 때와 마찬가지로 **「작성자가 제공한 자격 증명」** 그대로 둡니다. 만든 사람의 연결로 도구가 돌아간다는 뜻입니다. 실제로 배포할 때는 **「최종 사용자 자격 증명」** 을 쓰는 것이 일반적입니다 — 쓰는 사람이 각자 자기 권한으로 접근해야 남의 파일을 보게 되지 않기 때문입니다. 오늘은 **로그인 알림창이 자주 뜨는 것을 줄이려고** 작성자 쪽으로 둡니다.
    ...
    {: .note }
    기본 설명은 「테이블에 있는 행을 나열합니다」입니다. 틀린 말은 아니지만 우리 질문과 이어지지 않습니다. `지원단가`·`항목명` 같은 **우리 쪽 말**이 들어가야 걸립니다.
...
8. **입력** 항목으로 내려갑니다. 네 칸을 아래처럼 채웁니다.
    ...
    | File | 사용자 지정 값 | 본인이 올린 `자기계발비_기준표.xlsx` |
    | Table | **사용자 지정 값** | `지원기준` |

    `File` 칸은 오른쪽 **폴더 아이콘**을 눌러 목록에서 고릅니다. 직접 입력하지 않습니다.

    ![File 칸 — 폴더 아이콘으로 자기계발비_기준표.xlsx 고르기](../assets/lab5/lab5-07b.png)

    `Table` 칸은 채우는 방법을 **사용자 지정 값**으로 바꾼 뒤 드롭다운을 엽니다. 처음에는 **「Table에 대한 제안 사항을 찾을 수 없습니다」**가 뜹니다. 목록 아래의 **새로 고침**을 누릅니다.
...
9. **새 테스트 세션**에서 물어봅니다.
    ...
    **완료 기준**: `100,000` 이 응답에 나오고, **계약 테스트 창이 저절로 열리면서** 왼쪽에 `테이블에 있는 행 나열` 이 **커넥터 동작**으로 기록된다.

    ![지원단가 조회 응답과 도구 호출 상세](../assets/lab5/lab5-09.png)

    {: .note }
    도구가 불리면 **계약 테스트 창**이 저절로 펼쳐집니다. 따로 열지 않아도 됩니다.

    {: .important }
    **토픽 때보다 보이는 것이 많습니다.** 왼쪽 상자를 누르면 7번에 쓴 **설명**, 8번에 지정한 **입력**, 그리고 도구가 돌려준 **출력**이 다 나옵니다. [Lab 4](./lab4.html) 24번에서는 설명까지만 보였습니다.

    {: .note }
    `Table` 칸에 8번에서 고른 `지원기준` 대신 **긴 코드**가 들어가 있습니다. `Location`·`Document Library`·`File` 도 마찬가지입니다. 화면에서 고른 이름이 내부 식별자로 바뀌어 전달된 것입니다. 값이 이상해 보여도 정상입니다.

    {: .note }
    답 끝에 `(근거: 자기계발비 지원단가 기준표)` 가 붙어 있습니다. [Lab 2](./lab2.html)에서 지침에 쓴 **근거 표기 규칙이 도구에도 그대로 적용**된 것입니다.
...
10. 호출을 확실하게 만듭니다. **지침**의 「답변 원칙」 아래에 한 줄을 더합니다. 도구 이름 자리에서 **`/`** 를 누르면 목록이 뜹니다.
    ...
    **완료 기준**: 도구 이름이 **아이콘 붙은 칩**으로 바뀐다. 저장합니다.
    ...
    {: .important }
    **[Lab 4](./lab4.html) 12번에서 토픽을 지목한 것과 같은 자리, 같은 방법입니다.** 지침에서 무엇을 부를지 적는 문법이 토픽과 도구에 공통입니다. 
...
11. **새 테스트 세션**에서 10번을 다시 하고, 이번에는 표에 없는 코드도 물어봅니다.
    ...
    **완료 기준**: 앞은 `100,000` 이 나오고 도구 호출이 기록된다. 뒤는 **없다고 답하고 숫자를 만들지 않는다.**
    ...
    {: .important }
    **없다고만 하지 않고 등록된 항목명을 전부 보여줍니다.** 이 도구가 `헬스장 이용권` 한 줄을 찾아본 것이 아니라 **표를 통째로 읽어 왔기** 때문에 가능한 답입니다. 이름 그대로 「테이블에 있는 행 나열」입니다.
...
15. 13번 대화가 어떻게 처리됐는지 **활동** 탭에서 봅니다. 방금 대화를 열면 왼쪽에 처리 순서가 남아 있습니다.
    ...
    **완료 기준**: 커넥터 동작 상자는 `테이블에 있는 행 나열` **하나뿐**이고, 그 아래 **추론**에 곱셈이 문장으로 적혀 있다.
...
20. 도구의 **설명**을 아래로 바꿉니다.
    ...
    ![쓰기 도구 — 설명과 추가 세부 정보](../assets/lab5/lab5-20.png)
...
21. **새 테스트 세션**에서 신청하고 기록까지 시킵니다.
    ...
    **완료 기준**: 기록했다는 응답이 나오고, 계약 테스트 창에 상자가 **둘**이다 — `테이블에 있는 행 나열` 과 `테이블에 행 추가`.

    ![기록 응답 — 상자 두 개와 접수 내역](../assets/lab5/lab5-21.png)

    {: .note }
    응답의 **신청자** 칸에 계정 정보가 들어가 있습니다. 우리는 알려준 적이 없습니다. 모델이 지금 로그인한 계정에서 가져다 쓴 것인데, **어디서 온 값인지 지침에도 도구 설명에도 적혀 있지 않습니다.** [Lab 6](./lab6.html)에서는 이 값을 명시적으로 넘깁니다.
...
23. 두 가지를 시켜 보고 결과를 견줍니다. 먼저 **없는 기능**을 시킵니다.
    ...
    **완료 기준**: 무엇을 쓸지 되묻지 않고 **다섯 건이 한 번에** 처리된다. 계약 테스트 창에 `테이블에 행 추가` 가 **다섯 번 더** 붙어 상자가 모두 일곱이 된다.
...
24. **OneDrive에서 Excel 파일을 열어** 확인합니다.
    ...
    **완료 기준**: 샘플 세 줄 아래로 **여섯 줄**이 쌓여 있다(22번 한 줄 + 방금 다섯 줄). 그리고 **여섯 줄 전부 22번과 같은 방식으로 어긋나 있다.**

    ![샘플 세 줄 아래로 쌓인 여섯 줄 — 값이 전부 같은 방식으로 밀려 있다](../assets/lab5/lab5-24.png)

    {: .important }
    **화면에는 맞게 쓰고, 표에는 틀리게 썼습니다.** 23번의 답변에서 `온라인 강의 · 150,000원`은 정확했습니다. 그런데 표에는 `신청자`에 `150000`이, `항목명`에 `접수`가, `수량`에 `온라인 강의`가 들어갔습니다. **아는 것과 옮기는 것은 다른 일입니다.**

    {: .important }
    **어긋나는 방식이 여섯 줄 모두 같습니다.** 한 번 삐끗한 것이 아니라 **매번 같은 자리로 밀립니다.** 값이 우연히 어긋난 것이라면 줄마다 다르게 나왔을 것입니다.
