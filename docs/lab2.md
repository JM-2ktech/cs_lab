...
<!-- 저작 메모(2026-08-11 통합): 구 Lab 2(지식) + 구 Lab 3(지침 A/B/C·범위 밖 질문)을 합쳤다.
     ...
     구 Lab 3에서 옮겨간 것:
       ...
       - 스텝 11~14 토크나이저·크레딧 → `reference-inside-the-agent.md` 2부.
         비용은 성격이 다르고 외부 웹툴에 의존한다. 랩이 아니라 설명+실습이 맞다.
       - 스텝 6 범위 밖 질문(날씨) → 이 랩 스텝 9. Lab 5와의 대구가 여기 걸려 있어 반드시 유지.
     구 Lab 2에서 옮겨간 것:
       - 스텝 7 RAG 수동 매칭 → `reference-inside-the-agent.md` 1부.
     ...
     2026-08-11 삭제: "테스트 채팅은 무료입니다. 마음껏 시도해도 되는 이유입니다" 팁.
       ...
       (3) 뒷부분(에이전트 흐름 안 프롬프트는 크레딧 소모)은 이 랩에서 흐름을 쓰지 않으므로
           자리가 아니다. 비용 이야기는 reference-inside-the-agent.md 2부가 소관이다.
     ...
     2026-08-11 실측 정정: 추천 프롬프트는 Copilot Studio 테스트 패널에 나오지 않는다.
       ...
       ※ MS 문서: 게시 후에도 캐시·세션 때문에 즉시 안 보일 수 있다(브라우저/Teams/CDN).
         ...
         Lab 7에서 게시할 때 이 문제가 나올 수 있으니 그쪽 저작 시 참고할 것.

     2026-08-11 실측 정정(2): 스텝 순서를 바꿨다. 설정을 먼저 다 하고 기준선 질문을 한 번만 던진다.
       원인 — 「웹의 정보 사용」은 **기본값이 끄기**다. 손대기 전 컷(lab2-11)에서 이미 꺼져 있다.
       그래서 "웹 켜진 상태 → 끈 상태" 대비가 애초에 성립하지 않았다. 소재가 가상 회사라
       웹을 일부러 켜도 답이 달라지지 않는다(lab2-13에서 확인: 웹 없이도 "찾지 못했다"고 답한다).
       같은 질문 4회 → 3회로 줄었다(기준선 / 참조 자료 후 / 지침 후).
     2026-08-11 발견: 설정 > 생성형 AI에 **연결된 에이전트 > 다른 에이전트가 이 에이전트에
       연결하여 사용하도록 허용** 토글이 있다. Lab 7에서 이 에이전트를 자식으로 붙이려면 필요하다.
     ...
     2026-08-11 모델 목록 실측(lab2-a01 컷, 변동 주의):
     ...
     ★ 「더 해볼 것」 절의 수치는 제작자 계측(2026-08-11): 지침을 통째로 비우고 참조 자료만
       ...
       ⚠️ 지침을 지운 채로 두면 Lab 7이 깨진다. 되돌리고 재게시하라는 경고를 반드시 유지할 것.
     ...
     ★ 부록 실측(lab2-a03, GPT-5 Chat): GPT 계열이 퇴직금을 근로기준법으로 답하면서
     ...
     2026-08-11 실측 정정(3): 스텝 18~24 촬영분 반영.
       - 상태 라벨이 「준비됨」이 아니라 **「준비」**다. lab2.md·참고 페이지 전부 정정했다.
...
{: .time }
10분 · 읽고 넘어갑니다. **타이머 밖입니다.** 손을 움직이는 것은 「단계」부터입니다.

### 우리는 AI와 직접 대화하고 있지 않다

ChatGPT에 질문을 입력하면 내 말이 AI에게 곧장 가는 것처럼 보입니다. 실제로는 그렇지 않습니다. 나와 AI 사이에는 **오케스트레이터**라는 중개자가 있습니다.

ChatGPT, M365 Copilot, Gemini, Claude — 지금 쓰이는 AI 챗봇은 예외 없이 이 구조입니다. AI 모델이 혼자 사용자를 상대하는 서비스는 사실상 없습니다.

한 번 질문할 때 실제로 벌어지는 일은 이렇습니다.

```mermaid
flowchart TD
    U[사용자 질문] --> O[오케스트레이터]
    O --> B["① 지침을 질문 앞에 붙인다<br>② 필요한 지식을 찾아 온다<br>③ 필요한 도구를 호출한다<br>④ 이전 대화를 합친다"]
    B --> L["LLM — 문장을 만든다"]
    L --> O2[오케스트레이터가 다듬는다]
    O2 --> A[사용자에게 표시]
```

가운데 네 가지가 오케스트레이터의 일입니다. **LLM은 마지막에 문장을 만드는 자리에만 등장합니다.**

### 오케스트레이터가 하는 네 가지

**① 성격을 정한다 — 시스템 프롬프트**

질문을 LLM에 넘기기 전에, 오케스트레이터는 그 앞에 "너는 누구고 무엇을 해야 하는가"를 먼저 붙입니다. M365 Copilot이라면 이런 문장이 사용자 눈에 보이지 않는 자리에 깔려 있습니다.

```
너는 Microsoft 365 Copilot이다. 사용자의 업무를 돕는 것이 목적이다.
정치적 의견은 말하지 마라. 답변은 한국어로 하라.
```

같은 모델이라도 이 문장이 다르면 전혀 다르게 행동합니다. **우리가 쓰는 「지침」이 바로 이 자리에 들어갑니다.**

**② 참고할 것을 고른다 — 지식**

LLM은 회사 규정을 모릅니다. 학습할 때 본 적이 없기 때문입니다. 그래서 오케스트레이터가 문서에서 필요한 대목을 찾아 질문과 함께 넘겨줍니다. 이번 랩에서 파일 4개를 올리는 일이 여기에 해당합니다.

**③ 수단을 동원한다 — 도구**

"오늘 환율 알려줘"에 답하려면 어딘가에서 환율을 가져와야 합니다. LLM은 인터넷에 접속할 수 없습니다. 오케스트레이터가 검색 도구를 호출해 결과를 받아다 넘겨주는 것입니다. 파일을 읽는 것도, 메일을 조회하는 것도, 코드를 실행하는 것도 전부 마찬가지입니다.

**④ 맥락을 관리한다 — 대화 기록**

"아까 그 파일"이라고만 해도 알아듣는 이유는 오케스트레이터가 이전 대화를 함께 넘기기 때문입니다. 사용자의 말투나 선호도 여기서 반영됩니다.

### LLM이 하는 일은 생각보다 좁다

위 네 가지 중 **LLM이 스스로 하는 것은 하나도 없습니다.** LLM은 주어진 글을 읽고 그럴듯한 다음 문장을 만드는 엔진입니다. 검색하고, 파일을 읽고, 판단해서 도구를 고르는 일은 전부 오케스트레이터의 몫입니다.

AI가 없는 사실을 지어내는 현상도 여기서 나옵니다. LLM은 본질적으로 "그럴듯한 다음 말"을 만들 뿐이라, 근거가 없으면 근거 없이 그럴듯한 문장을 만듭니다. **이걸 줄이는 것이 오케스트레이터의 일이고, 수단이 지식 연결과 지침입니다.** 이번 랩에서 하는 일이 정확히 그 둘입니다.

### 어제 이미 네 가지를 다 만졌습니다

[Lab 1](./lab1.html)에서 겪은 일들이 각각 무엇이었는지 이제 이름을 붙일 수 있습니다.

| 오케스트레이터의 일 | Lab 1에서 겪은 것 |
|---|---|
| ① 성격 — 지침 | 스텝 16. 같은 지침, 같은 질문인데 결과가 매번 달랐습니다. 지침은 **부탁**이지 강제가 아니었습니다 |
| ② 참고 대상 — 지식 | 스텝 7. Copilot은 원래 내 메일을 볼 수 있는데도, 「내 이메일」을 켜기 전까지는 보지 않았습니다 |
| ③ 수단 — 도구 | 스텝 17. "그냥 보내줘"에 안 보낸 것은 잘 막아서가 아니라 **보낼 손이 없어서**였습니다 |
| ④ 맥락 — 대화 | 스텝 12. 같은 채팅에서 계속 물으면 앞 대화가 결과에 섞여, 새 채팅으로 테스트해야 했습니다 |

### 같은 모델, 다른 오케스트레이터

셋 다 같은 AI 모델을 쓸 수 있습니다. 결과가 다른 이유는 모델이 아니라 오케스트레이터입니다.

| | ChatGPT | M365 Copilot | 오늘 만들 에이전트 |
|---|---|---|---|
| **지침** | 범용 어시스턴트 | 업무 도우미 | 한별소프트 복리후생 안내 |
| **볼 수 있는 것** | 사용자가 준 것만 | 메일·파일·일정·Teams | 사내 규정 문서 4종 |
| **쓸 수 있는 수단** | 웹 검색·코드 실행 | + M365 데이터 | + 토픽·커넥터·흐름 |
| **결과** | 일반적인 답 | 내 업무 맥락이 섞인 답 | **한별소프트 규정에 맞는 답** |

### 왜 따로 만들어야 하나

M365 Copilot이 이미 잘 답하는데 에이전트를 따로 만드는 이유는 **Copilot의 오케스트레이터가 범용이기 때문**입니다. 회사의 복리후생 규정을 모르고, 모르는 것을 물으면 일반론으로 답하거나 지어냅니다.

우리 규정만 보고, 우리 말투로, 모르는 것은 모른다고 답하게 하려면 **그렇게 행동하는 오케스트레이터를 따로 만들어야** 합니다. 그게 에이전트입니다.

{: .important }
**문장을 만드는 것은 AI가 하지만, 무엇을 보고 어떻게 답할지 정하는 것은 우리입니다.** 오늘 하루 동안 하는 일은 결국 하나입니다 — 나만의 오케스트레이터를 설계하는 것.

### 오늘 남은 랩이 채우는 자리

| 오케스트레이터의 일 | 실습에서 하는 것 | 어디서 |
|---|---|---|
| 성격 | 지침 작성 | **이번 랩** |
| 참고 대상 | 지식 소스 연결 | **이번 랩** |
| 정해진 절차 | 토픽 만들기 | [Lab 4](./lab4.html) |
| 수단 | 커넥터 붙이기 | [Lab 5](./lab5.html) |
| 다른 에이전트 부르기 | 연결된 에이전트 | [Lab 6](./lab6.html) |
...
| 용어 | 화면에서 | 한 줄 |
|---|---|---|
| [지식 (Knowledge)](./glossary.html#knowledge) | 상단 **참조 자료** 탭 | 답변의 **근거 문서**. 화면 라벨은 「참조 자료」다. 오늘은 복리후생 문서 4종 |
...
| 근거 없는 응답 허용 | 설정 토글 | 켜 두어야 "그건 모른다"고 **스스로 설명**한다. 끄면 대화가 오류로 끊긴다 |
| 게시 (Publish) | 우측 상단 버튼 | 저장한 내용을 실제로 적용하는 동작. **저장만으로는 반영되지 않는다** |
| 솔루션 (Solution) | 만들 때 선택 | 만든 것들을 **배포하려고 묶어 두는 상자**. 오늘은 기본값 그대로 두고 넘어갑니다 |
...
- 지식 소스 **4종**을 내려받아 바탕화면에 모아 두세요. 파일 이름을 누르면 받아집니다.
    ...
    {: .note }
    네 문서 모두 **가상 데모 자료**입니다. 회사명·금액·제도는 실제와 무관합니다.
...
>
> **다른 환경에서 만들면 오늘 남은 랩이 전부 막힙니다.** 에이전트는 만들어지지만 Lab 7에서 이 에이전트를 도구로 붙일 때 목록에 보이지 않습니다. 그때는 처음부터 다시 만들어야 합니다.
...
1. [copilotstudio.microsoft.com](https://copilotstudio.microsoft.com)에 본인 M365 계정으로 로그인합니다. **완료 기준**: Copilot Studio 홈 화면이 열린다.
    ...
    ![Copilot Studio 로그인 직후 홈](../assets/lab2/lab2-01.png)

2. 우측 상단에서 **환경**이 올바른지 확인합니다. **완료 기준**: 강사가 안내한 환경 이름이 표시된다.

    ![환경 선택 드롭다운](../assets/lab2/lab2-02.png)

3. 왼쪽 메뉴 **에이전트**를 클릭하고, 오른쪽 위 **+ 빈 에이전트 만들기**를 누릅니다.

    ![에이전트 메뉴에서 빈 에이전트 만들기](../assets/lab2/lab2-03.png)

4. **에이전트 이름 지정** 창이 뜹니다. 이름에 아래를 입력합니다. `HGD` 자리에는 **본인 영문 이니셜**을 넣습니다(홍길동 → `HGD`).
    ...
    {: .warning }
    **이니셜을 빼고 만들지 마세요.** 오늘 교육생 전원이 **같은 환경**에서 작업합니다. 이름이 같으면 객체명이 겹쳐 만들기 자체가 막히거나, [Lab 6](./lab6.html)에서 목록에서 골라 붙일 때 어느 것이 내 것인지 알 수 없습니다. 옆자리와 이니셜이 같으면 숫자를 하나 더 붙이세요(`HGD2`).

    ![이름 입력 후 에이전트 설정 펼치기](../assets/lab2/lab2-04.png)

5. 펼쳐진 설정에서 **스키마 이름**을 채웁니다. 앞의 `crb9e_` 같은 접두사는 환경이 자동으로 붙이는 것이니 건드리지 말고, 뒤쪽만 아래처럼 입력합니다.

    ```
    WelfareAgentHGD
    ```

    같은 화면의 **솔루션**은 기본값 그대로 둡니다. 다 됐으면 **만들기**를 누릅니다.

    ![스키마 이름 입력 · 솔루션 기본값](../assets/lab2/lab2-05.png)

    {: .warning }
    스키마 이름은 만든 뒤에 바꾸기 번거롭습니다. 지금 채워 두세요. 여기에도 이니셜을 붙입니다.

    {: .note }
    **솔루션**은 만든 것들을 배포하려고 묶어 두는 상자입니다. 오늘은 기본값(`Common Data Services Default Solution`)으로 충분합니다.
...
6. 생성이 끝나면 개요 화면이 열립니다. **상단 탭**을 훑어봅니다 — 오케스트레이터가 하는 일이 화면에서는 이렇게 나뉘어 있습니다.
    ...
    **완료 기준**: 표의 각 항목이 화면의 어느 탭인지 찾을 수 있다.
    ...
    ![개요 화면 탭 구조](../assets/lab2/lab2-06.png)

    {: .note }
    화면 폭에 따라 상단 탭 일부가 접혀 있습니다. 접힌 표시를 누르면 **도구 · 에이전트 · 토픽 · 활동 · 평가 · 모니터 · 채널**이 나옵니다. 오늘 나머지 랩에서 이 중 몇 개를 채웁니다.

7. **설명**에 아래를 입력합니다. Lab 1의 Agent Builder에서 「에이전트 설명하기」와 같은 자리입니다.

    ```
    한별소프트의 복리후생·휴가·경조사 제도를 사내 문서에 근거해 안내합니다.
    ```

    ![세부 정보 편집 진입](../assets/lab2/lab2-07.png)

    입력한 뒤 **저장**을 누릅니다.

    ![설명 입력 후 저장](../assets/lab2/lab2-07b.png)

8. **추천 프롬프트**에서 **+ 추천 프롬프트 추가**를 누릅니다.

    ![추천 프롬프트 추가](../assets/lab2/lab2-08.png)
    ...
    **완료 기준**: 목록에 2줄이 저장된다.
    ...
    ![추천 프롬프트 2개 저장](../assets/lab2/lab2-08b.png)

    {: .important }
    **등록해도 지금 화면에서는 보이지 않습니다.** 섹션 설명에 적혀 있듯 추천 프롬프트는 **Teams와 Microsoft 365 Copilot 채널**에서 대화를 시작하는 카드입니다. 게시해서 그 채널에 올라간 뒤에야 나타납니다. Lab 1의 Agent Builder에서는 미리보기에 바로 보였는데, 여기서는 만드는 곳과 쓰이는 곳이 다릅니다.

9. 상단의 **설정**을 누르고 **생성형 AI**로 들어갑니다.

    ![설정 진입](../assets/lab2/lab2-09.png)
...
10. 맨 위 **오케스트레이션**이 **예 — 응답은 사용 가능한 도구와 참조 자료를 적절히 활용해 동적으로 진행됩니다**로 되어 있는지 확인합니다. 바로 아래 **연결된 에이전트**의 **다른 에이전트가 이 에이전트에 연결하여 사용하도록 허용**도 **켜기**인지 봅니다. 둘 다 기본값이므로 대개 그대로 두면 됩니다.
    ...
    ![오케스트레이션 · 연결된 에이전트 허용](../assets/lab2/lab2-10.png)

    {: .note }
    **연결된 에이전트 허용**은 [Lab 6](./lab6.html)에서 이 에이전트를 다른 에이전트의 도구로 붙일 때 필요합니다. 지금 꺼두면 그때 목록에 나타나지 않습니다.

11. 아래로 내려가 **지식** 항목의 **근거 없는 응답 허용하기**가 **켜기**인지 확인합니다.

    ![근거 없는 응답 허용하기 = 켜기](../assets/lab2/lab2-11.png)

    {: .important }
    **이 토글을 끄면 오늘의 마지막 검증이 성립하지 않습니다.** 켜 두어야 에이전트가 "그건 개인 데이터라 제가 알 수 없습니다"라고 **스스로 설명**합니다. 끄면 같은 상황에서 대화가 오류 응답으로 끊겨 버려서, 에이전트가 경계를 아는 것인지 그냥 막힌 것인지 구분할 수 없습니다.
...
12. 같은 화면에서 **웹의 정보 사용**이 **끄기**인지 확인합니다. 기본값이 끄기지만 켜져 있으면 끕니다.
    ...
    ![웹의 정보 사용 = 끄기](../assets/lab2/lab2-12.png)

    더 내려가 **사용자 피드백**의 **에이전트 메시지에 대한 사용자 반응 수집**도 **끄기**로 둡니다.

    ![사용자 반응 수집 = 끄기](../assets/lab2/lab2-12b.png)

    {: .important }
    **웹의 정보 사용을 켜면 오늘의 검증이 성립하지 않습니다.** 문서에 없는 내용도 인터넷에서 그럴듯하게 가져오기 때문에, 뒤에서 에이전트가 답을 냈을 때 그게 우리 문서로 답한 것인지 웹에서 주워온 것인지 구분할 수 없습니다. 오케스트레이터가 쓸 수단을 **우리가 정한 것만으로 좁히는** 작업입니다.
...
13. 오른쪽 **테스트** 패널에 아래 문장을 입력합니다. 아직 참조 자료도 지침도 넣지 않은 상태입니다.
    ...
    **관찰**: 답이 나오는지, 나온다면 무슨 근거로 답하는지 확인합니다. 이 답이 **기준선**입니다 — 앞으로 같은 질문을 두 번 더 던져 무엇이 달라지는지 봅니다.
    ...
    ![참조 자료·지침 없는 상태의 응답](../assets/lab2/lab2-13.png)

    {: .note }
    아직 아무 근거도 주지 않았으므로 "찾지 못했다"는 취지의 답이 정상입니다. 회사 규정은 LLM이 학습할 때 본 적이 없고, 웹 검색도 꺼져 있기 때문입니다.

14. 상단 **참조 자료** 탭으로 이동합니다. 「참조 자료 원본 추가」 화면에서 **+ 참조 자료 추가**를 누릅니다. 여기가 [지식](./glossary.html#knowledge)을 넣는 자리입니다.

    ![참조 자료 추가 메뉴](../assets/lab2/lab2-14.png)

15. 준비된 **4개 파일을 한 번에** 올립니다. **완료 기준**: 목록에 4건이 모두 뜬다.

    ![참조 자료 4종 업로드 직후](../assets/lab2/lab2-15.png)

    {: .warning }
    한 개씩 올리면 인덱싱 대기를 네 번 하게 됩니다. **반드시 4개를 한 번에** 올리세요.

16. 인덱싱이 도는 동안 각 문서에 **설명**을 붙입니다. 파일 행 오른쪽 `...` 메뉴에서 **편집**을 고릅니다.

    ![파일 목록에서 편집 진입](../assets/lab2/lab2-16.png)
    ...
    **설명** 칸에 아래를 넣고 **저장**합니다. 네 파일 모두 같은 방식으로 반복합니다.
    ...
    ![설명 입력 후 저장](../assets/lab2/lab2-16b.png)

    {: .warning }
    **설명은 필수 항목입니다**(칸 이름 옆에 `*`). 비워 두면 저장되지 않습니다.

    {: .note }
    이 설명은 장식이 아닙니다. 오케스트레이터가 **어느 문서를 뒤질지 고를 때** 참고합니다. 문서가 늘어날수록 설명의 정확도가 검색 품질을 좌우합니다.

17. 인덱싱에는 **평균 5분, 길면 10분**이 걸립니다. 그동안 [참고. 에이전트 안에서 일어나는 일](./reference-inside-the-agent.html)의 **1부**를 읽습니다. 지금 화면 뒤에서 벌어지는 일이 거기 있습니다.

    {: .note }
    기다리는 동안 다른 탭을 만져도 됩니다. 다만 **참조 자료를 더 올리거나 지우지는 마세요** — 인덱싱이 처음부터 다시 돕니다.

18. 참조 자료 상태가 모두 **준비**로 바뀌었는지 확인합니다. **완료 기준**: 네 파일이 전부 준비.

    ![참조 자료 4종 "준비" 상태](../assets/lab2/lab2-18.png)

    {: .warning }
    **준비**로 바뀌기 전에 질문하면 "모른다"고 답합니다. 파일이 올라가 있어도 아직 검색 대상이 아니기 때문입니다.

19. **새 테스트 세션**에서 9번과 같은 질문을 세 번째로 입력합니다.
    ...
    **완료 기준(필수)**: `연 120만 원`과 사용처가 문서 그대로 나온다.
    ...
    ![참조 자료 등록 후의 응답](../assets/lab2/lab2-19.png)

    {: .important }
    **지침은 아직 한 글자도 쓰지 않았습니다.** 파일 4개를 올렸을 뿐인데 답이 달라졌습니다. 오케스트레이터가 문서에서 해당 대목을 찾아 LLM에 함께 넘겨준 결과입니다.

20. 답변 아래의 **참조**를 눌러 펼칩니다. 어느 문서가 근거였는지 나옵니다.

    **완료 기준**: 근거 문서가 표시된다. 대개 「01. 복리후생 제도 개요」와 「04. 사내복지·자주 묻는 질문」 **두 건**이 잡힌다.

    ![참조 펼침 — 근거 문서 두 건](../assets/lab2/lab2-20.png)

    {: .important }
    **한 답변이 문서 하나에서만 나오는 것이 아닙니다.** 별포인트는 「01」에 제도가, 「04」에 FAQ가 있어서 양쪽 조각이 함께 모였습니다. 문장마다 붙은 작은 번호가 **그 문장이 어느 문서에서 왔는지**를 가리킵니다.

    {: .note }
    LLM이 문서 네 개를 통째로 읽은 것이 아닙니다. 여기 표시된 **몇 줄만** 받아서 답을 만들었습니다. 참고 페이지 1부에서 손으로 해본 검색·그라운딩이 실제로 일어난 자리입니다.

    {: .warning }
    **참조가 붙었다고 그 문서에 그 내용이 있는 것은 아닙니다.** 눌러서 어느 대목인지 확인하는 습관이 필요합니다. 부록에서 그 이유를 직접 보게 됩니다.
...
21. 나머지 문서도 제대로 붙었는지 하나씩 확인합니다. 먼저 「02. 휴가·근태 안내」입니다.
    ...
    **완료 기준**: `매월 개근 시 1일씩 발생(최대 11일)`이 나온다.
    ...
    ![02 문서 겨냥 질문의 응답](../assets/lab2/lab2-21.png)
...
22. 「03. 경조사·생활지원」입니다.
    ...
    **완료 기준**: 경조금 `100만 원`, 경조휴가 `5일`이 나온다.
    ...
    ![03 문서 겨냥 질문의 응답](../assets/lab2/lab2-22.png)
...
23. 「04. 사내복지·자주 묻는 질문」입니다.
    ...
    **완료 기준**: `평일 07:00~21:00`이 나온다.
    ...
    ![04 문서 겨냥 질문의 응답](../assets/lab2/lab2-23.png)

    {: .note }
    네 문서가 모두 검색되고 있음을 확인했습니다. 준비 절의 표에서 각 파일이 무엇을 담고 있는지 다시 보면, 방금 던진 질문들이 어느 문서를 겨냥한 것이었는지 보입니다.

24. **지침** 항목 오른쪽의 **편집**을 누릅니다.

    ![지침 편집 진입](../assets/lab2/lab2-24.png)
    ...
    ![지침 붙여넣기](../assets/lab2/lab2-24b.png)

    {: .note }
    세 덩어리로 나눠 썼습니다. **역할**은 이 에이전트가 누구인지, **답변 원칙**은 어떻게 답할지, **경계**는 무엇을 말하지 않을지입니다. 지침이 길어질수록 이렇게 나눠 두어야 나중에 어디를 고칠지 찾을 수 있습니다.

25. 저장한 뒤 우측 상단 **게시**를 누릅니다.

    ![게시 버튼](../assets/lab2/lab2-25.png)

    **이 에이전트 게시** 창이 뜹니다. **최신 버전 강제 적용**은 체크하지 않은 채 **게시**를 누릅니다. **완료 기준**: 게시 완료 표시가 뜬다.

    ![게시 대화상자](../assets/lab2/lab2-25b.png)

    {: .note }
    **최신 버전 강제 적용**은 Teams처럼 대화가 계속 이어지는 채널에서 **진행 중인 대화에도 새 버전을 바로 밀어 넣는** 옵션입니다. 켜면 최신 상태가 즉시 퍼지지만 진행 중인 대화가 끊깁니다. 오늘은 채널에 올리지 않으므로 꺼둡니다.

    {: .note }
    `Agent status`에 **1 Warning**이 떠 있어도 게시됩니다. 「아직 평가를 돌리지 않았다」는 안내이지 오류가 아닙니다.
...
26. **새 테스트 세션**에서 아래 **세 질문**을 차례로 입력합니다. 셋의 성격이 각각 다릅니다.
    ...
    **완료 기준**: `연 120만 원`과 사용처가 나오고, 답변 끝에 근거 문서가 표기된다.
    ...
    ![문서에 있는 것 — 별포인트](../assets/lab2/lab2-26.png)
    ...
    **완료 기준**: 숫자를 말하지 않고 **한별포털 > 근태**와 **피플팀(내선 1000)**으로 안내한다.
    ...
    ![기준은 있고 데이터는 없는 것 — 남은 연차](../assets/lab2/lab2-26b.png)
    ...
    **완료 기준**: 근로기준법 계산식을 늘어놓지 않고, **사내 자료에서 찾을 수 없다**고 답한 뒤 피플팀으로 안내한다.
    ...
    ![문서에 없는 것 — 퇴직금](../assets/lab2/lab2-26c.png)

    {: .important }
    **셋을 가르는 것이 이 랩의 핵심입니다.** 지식에 넣은 문서는 **기준(제도)**이지 **데이터(내 잔여 연차)**가 아니고, 기준에도 없는 것(퇴직금)은 모델이 알고 있어도 말하지 않아야 합니다. 지식은 에이전트가 **무엇을 아는가**를 정하고, 지침은 **무엇을 말해도 되는가**를 정합니다.

    {: .warning }
    **지금 쓰는 모델은 이 셋을 지침 없이도 상당 부분 지킵니다.** 그렇다고 지침이 필요 없는 것은 아닙니다 — 부록에서 모델만 바꿔보면 같은 지침이 얼마나 다르게 지켜지는지 드러납니다.
...
27. **한 문서로는 답할 수 없는** 질문을 입력합니다.
    ...
    **완료 기준**: 리프레시 휴가 **5일**(근속 3·5·10년)과 전세자금 **최대 5,000만 원**(근속 2년 이상)을 함께 답하고, 근거 문서를 **둘 다** 표기한다.
    ...
    ![교차 질의 응답 — 리프레시 휴가](../assets/lab2/lab2-27.png)

    ![교차 질의 응답 — 전세자금과 근거 문서 2건](../assets/lab2/lab2-27b.png)

    {: .note }
    리프레시 휴가는 「02. 휴가·근태 안내」에, 전세자금은 「03. 경조사·생활지원」에 있습니다. 어느 한쪽만 봐서는 답이 나오지 않습니다. 오케스트레이터가 **두 문서에서 조각을 모아 붙인** 것입니다. 참조를 펼치면 두 문서가 나란히 표시됩니다.

    {: .note }
    답에 「근속 4년이시므로 신청 대상에 해당됩니다」가 붙었다면 눈여겨보세요. 문서에서 찾아 옮긴 것이 아니라 **조건(근속 2년 이상)과 질문의 값(4년)을 대조한 것**입니다. 검색만으로는 나오지 않는 답입니다.
...
28. 마지막으로 **범위 밖 질문**을 하나 입력합니다.
    ...
    **완료 기준**: 에이전트가 어떻게 반응하는지 **관찰한다**(거절·안내·일반 답변 — 정답은 없습니다).
    ...
    ![범위 밖 질문에 대한 반응](../assets/lab2/lab2-28.png)

    {: .important }
    이 답은 [Lab 5](./lab5.html)에서 다시 씁니다. 거기서 똑같은 질문을 던지면 이번엔 답합니다 — 지식이나 지침을 바꿔서가 아니라 **도구를 하나 붙여서**입니다. 지식이 "무엇을 아는가"라면 도구는 "무엇을 할 수 있는가"입니다.
...
이 에이전트는 [Lab 4](./lab4.html)·[Lab 5](./lab5.html)·[Lab 6](./lab6.html)에서 계속 씁니다. 랩마다 지침이 조금씩 붙으므로, 랩이 끝날 때마다 지금 상태를 여기에 적어 둡니다. 따라오다 어긋난 것 같으면 이 블록으로 맞춥니다.
...
24번에서 붙인 그대로입니다. 이 랩에서는 더 손대지 않았습니다.

---

## 부록. 모델을 바꾸면 같은 지침이 다르게 지켜진다
...
1. **개요** 탭의 **에이전트의 모델 선택**에서 지금 어떤 모델을 쓰고 있는지 확인합니다.
    ...
    ![에이전트의 모델 선택](../assets/lab2/lab2-a01.png)
...
4. **새 테스트 세션**에서 똑같은 질문을 다시 입력합니다.
    ...
    **관찰**: 답이 달라지는지. 그리고 **근거를 어떻게 대는지.**
    ...
    ![모델을 바꾼 뒤의 응답](../assets/lab2/lab2-a03.png)
...
{: .warning }
**지침을 지웠다면 반드시 되돌려 놓고 다시 게시하세요.** 되돌릴 원문은 위 [「여기까지의 지침」](#여기까지의-지침)에 있습니다. [Lab 6](./lab6.html)에서 이 에이전트를 그대로 씁니다.
