...
11. 아래로 내려가 **지식** 항목의 **근거 없는 응답 허용하기**가 **켜기**인지 확인합니다.
    ...
    {: .important }
    **이 토글을 끄면 오늘의 마지막 검증이 성립하지 않습니다.** 켜 두어야 에이전트가 "그건 개인 데이터라 제가 알 수 없습니다"라고 **스스로 설명**합니다. 끄면 같은 상황에서 대화가 오류 응답으로 끊겨 버려서, 에이전트가 경계를 아는 것인지 그냥 막힌 것인지 구분할 수 없습니다.[^learn-ungrounded]
...
24. **지침** 항목 오른쪽의 **편집**을 누릅니다.
    ...
    {: .note }
    세 덩어리로 나눠 썼습니다. **역할**은 이 에이전트가 누구인지, **답변 원칙**은 어떻게 답할지, **경계**는 무엇을 말하지 않을지입니다. 지침이 길어질수록 이렇게 나눠 두어야 나중에 어디를 고칠지 찾을 수 있습니다. 이 모양을 쓰는 이유는 부록 [「지침을 어떻게 쓰는가」](./appendix/instructions-markdown.html)에 있습니다.
...
이 랩에는 [모델을 바꾸면 같은 지침이 다르게 지켜진다](./appendix/model.html) 부록이 있습니다. 랩 진행에 필요하지 않습니다 — [부록 모음](./appendix/)에서 골라 하세요.
